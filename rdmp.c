/*
  This converts a stream of ascii binary or hexadecimal digits and turns them into their equivilent bytes
*/
...
bin_to_byte (const char *bin) 
{
  ...
  for (int i = 0; i < 8; i++) {
      ...
      byte = (byte << 1) | (bin[i] - '0'); // ascii to integer conversion
    }
  return byte;
}
